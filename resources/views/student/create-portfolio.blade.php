@extends('layouts.app')
@section('title', 'Buat Portofolio — Scholarchive')

@section('content')
<section class="dk-page">
    <div class="dk-arc"></div>
    <div class="container" style="max-width:700px;position:relative;z-index:1;">
        <a href="{{ route('home') }}" class="dk-back-link animate-item delay-1">← Kembali ke Home</a>

        <div class="dk-card animate-item delay-2">
            <h2 class="dk-card-title">Unggah Karya Baru</h2>
            <form action="{{ route('student.portfolio.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="dk-form-group">
                    ...
                    <input type="text" name="title" class="dk-input" placeholder="e.g. Nova Brand Identity" value="{{ old('title') }}" required>
                </div>
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
                    <div class="dk-form-group">
                        ...
                        <select name="type" class="dk-input dk-select" required>
                            ...
                            <option value="fotografi" {{ old('type') == 'fotografi' ? 'selected' : '' }}>Fotografi</option>
                            <option value="tugas praktik" {{ old('type') == 'tugas praktik' ? 'selected' : '' }}>Tugas Praktik</option>
                            <option value="presentasi" {{ old('type') == 'presentasi' ? 'selected' : '' }}>Presentasi</option>
                            <option value="prototype" {{ old('type') == 'prototype' ? 'selected' : '' }}>Prototype</option>
                            <option value="aplikasi" {{ old('type') == 'aplikasi' ? 'selected' : '' }}>Aplikasi / Software</option>
                            <option value="blueprint" {{ old('type') == 'blueprint' ? 'selected' : '' }}>Blueprint / Cetak Biru</option>
                            <option value="laporan" {{ old('type') == 'laporan' ? 'selected' : '' }}>Laporan Proyek</option>
                            <option value="animasi" {{ old('type') == 'animasi' ? 'selected' : '' }}>Animasi</option>
                            <option value="film" {{ old('type') == 'film' ? 'selected' : '' }}>Film / Broadcasting</option>
                        </select>
                    </div>
                    <div class="dk-form-group">
                        ...
                        <div style="display:flex;flex-wrap:wrap;gap:0.5rem;">
                            ...
                            <div class="dk-chip-wrap">
                                <input type="checkbox" id="cat_{{ $cat->id }}" name="categories[]" value="{{ $cat->id }}" class="dk-chip-input" {{ (is_array(old('categories')) && in_array($cat->id, old('categories'))) ? 'checked' : '' }}>
                ...
                <div class="dk-form-group">
                    ...
                    <div class="dk-upload-zone" onclick="document.getElementById('file-upload').click()">
                        ...
                        <p style="font-size:0.75rem;color:#64748b;">PNG, JPG, PDF, MP4 (maks. 50MB)</p>
                        <input type="file" name="file" id="file-upload" style="display:none;" required onchange="document.getElementById('file-preview').textContent = this.files[0] ? this.files[0].name : ''">
                    </div>
